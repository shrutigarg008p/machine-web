    ...
    <div class="container mb-5">
        ...
        <div class="row">
            <div class="col-12 col-sm-8 col-md-6">
                ...
                <form class="submit-via-ajax" action="{{ route('admin.banner.update', [$banner->id]) }}" method="post" data-required_fields="{{\json_encode($required_fields)}}">
                    @csrf
                    {{-- @method('PATCH') --}}
                    ...
                    <div class="tab-content mt-4" id="langTabContent">
                        ...
                        @php
                            ...
                            $banner->setDefaultLocale($locale);
                        @endphp
                            ...
                            <div class="tab-pane fade {{$loop->first ? 'show active':''}}" id="tab{{ $locale }}" role="tabpanel"
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control input-title" id="title_{{ $locale }}"
                                        name="{{ $locale }}[title]" dir="{{$direction}}" value="{{$banner->title}}">
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="{{ $locale }}[short_description]"
                                        ...
                                        rows="3" dir="{{$direction}}">{{$banner->short_description}}</textarea>
                                </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="file" name="image" class="show_image_preview file-check-size-res" data-target="#preview-image" id="image" accept="image/png,image/gif,image/jpeg,image/jpg"data-min_width="1600" data-min_height="1050">
                        <img id="preview-image" src="{{storage_url($banner->image)}}" height="140" width="120" style="display: {{$banner->image ? '':'none'}}">
                    </div>
